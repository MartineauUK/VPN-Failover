...
#          VPN_Failover   1 once
#                         As above, but the script terminates immediately and if the VPN Client 1 connection is UP.
...
Check_VPN(){
	...
	if [ "$2" != "NOCURL" ];then
		...
		if [ $RC -eq 0 ];then
			...
			case "$2" in
				"$FORCE_WGET_12MB") SayT "cURL $(($(echo $RESULTS | cut -d',' -f5)/1000000))MByte transfer took:" $(printf "00:%05.2f secs @%6.0f B/sec" "$(echo $RESULTS | cut -d',' -f2)" "$(echo $RESULTS | cut -d',' -f3)")
									...
									;;
				"$FORCE_WGET_500B") SayT "cURL $(($(echo $RESULTS | cut -d',' -f5)))Byte transfer took:" $(printf "00:%05.2f secs @%6.0f B/sec" "$(echo $RESULTS | cut -d',' -f2)" "$(echo $RESULTS | cut -d',' -f3)")
...
while [ $# -gt 0 ]; do    # Until you run out of parameters . . .
	case $1 in
		...
		interval=*)	INTERVAL="$(echo "$1" | sed -n "s/^.*interval=//p" | awk '{print $1}' | grep -E "[[:digit:]]")"
...
while true
	do
		...
		case "$VPN_ID" in									# v1.08
			1)	FORCE_WGET_MIN_RATE=$MIN_CURLRATE_1			# The cURL rate specific to a VPN for file transfer...
				FORCE_WGET=$CURLSIZE1						# The cURL file to be measured
				;;
			2)	FORCE_WGET_MIN_RATE=$MIN_CURLRATE_2			# The cURL rate specific to a VPN for file transfer...
				FORCE_WGET=$CURLSIZE2						# The cURL file to be measured
				;;
			3)	FORCE_WGET_MIN_RATE=$MIN_CURLRATE_3			# The cURL rate specific to a VPN for file transfer...
				FORCE_WGET=$CURLSIZE3						# The cURL file to be measured
				;;
			4)	FORCE_WGET_MIN_RATE=$MIN_CURLRATE_4			# The cURL rate specific to a VPN for file transfer...
				FORCE_WGET=$CURLSIZE4						# The cURL file to be measured
				;;
			5)	FORCE_WGET_MIN_RATE=$MIN_CURLRATE_5			# The cURL rate specific to a VPN for file transfer...
