...
if [ "$1" == "status" ] || [ "$1" == "reset" ];then                                                     # v1.17
    ...
    for I in 1 2 3 4 5
        do
            ...
            TXT=
            INFO=
            PROCESS=$(ps -w | grep -v grep | grep "{" | grep -E "VPN_Failover\.sh $I")

            if [ -f /tmp/vpnclient${I}-VPNFailover ];then
                INFO=$(ls -l /tmp/vpnclient${I}-V* 2>/dev/null | sed 's/^.*root//')
                STAT=$(awk '{print $1}' /tmp/vpnclient${I}-VPNFailover)
...
VPNFAILOVER="/tmp/vpnclient"$VPN_ID"-VPNFailover"
##########################################################################################################################
# DON'T TRY DEBUGGING HERE.....It will fail as it will detect the debugging process instance!
##########################################################################################################################
...
if [ -n "$CMDDELAY" ];then
    SayT "Beginning VPN Client" $VPN_ID "connection status monitoring in" $(Convert_SECS_to_HHMMSS "$DELAY") ".....@"$(date -D '%s' -d "$(($(date +%s) + $DELAY))" +"%H:%M:%S")
    sleep $DELAY
...
while true; do
  ...
  if [ "$(nvram get "vpn_client"${VPN_ID}"_state")" != "$IS_VPN_UP" ] || [ "$PERFORMANCE" == "FAIL" ]; then
   ...
   "-1") REASON=$VPNSTATE";Unknown Error - Password/routing issue?" ;;
        ...
        if [ -z "$(echo "$IGNORE_VPN" | grep -oF "$NEW_VPN_ID")" ]; then
          ...
          if [ "$(nvram get "vpn_client"${NEW_VPN_ID}"_state")" != "$IS_VPN_UP" ]; then
            ...
            if [ "$(nvram get "vpn_client"$NEW_VPN_ID"_state")" == "-1" ]; then # v1.09
              ...
              [ -n "$CMDSENDMAIL" ] && SendMail "***ERROR VPN Client Monitor: VPN Client" $NEW_VPN_ID "FAILED to start (nvram  vpn_client${NEW_VPN_ID}_state reset)"
            fi
    ...
    # Check for external kill switch
    if [ ! -f $VPNFAILOVER ]; then
       Say "VPN Client Monitor: Monitoring VPN Client" $VPN_ID "terminated ('"$VPNFAILOVER"' not found)"
       #echo -e $cBYEL"\t\t"$(date +"%H:%M:%S")" Monitoring VPN Client" $VPN_ID "terminated ('"$VPNFAILOVER"' not found)\n"$cRESET
       break
    fi

  if [ -z "$VPN_NO_SLEEP" ]; then # v1.07
    echo -e $cBYEL"\n\t\tWill check VPN Client" $VPN_ID "connection status again in" $(Convert_SECS_to_HHMMSS "$INTERVAL") ".....@"$(date -D '%s' -d "$(($(date +%s) + $INTERVAL))" +"%H:%M:%S") # v1.08
    SayT "Will check VPN Client" $VPN_ID "connection status again in" $(Convert_SECS_to_HHMMSS "$INTERVAL") ".....@"$(date -D '%s' -d "$(($(date +%s) + $INTERVAL))" +"%H:%M:%S")
    sleep $INTERVAL
  else
    VPN_NO_SLEEP=
  fi
  ...
  # Check for external kill switch
  if [ ! -f $VPNFAILOVER ]; then
    SayT "VPN Client Monitor: Monitoring VPN Client" $VPN_ID "terminated ('"$VPNFAILOVER"' not found)"
    echo -e $cBYEL"\t\t"$(date +"%H:%M:%S")" Monitoring VPN Client" $VPN_ID "terminated ('"$VPNFAILOVER"' not found)\n"$cRESET
    break
  fi
